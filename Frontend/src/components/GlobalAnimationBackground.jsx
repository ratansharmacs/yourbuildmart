...
export default function GlobalAnimationBackground() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const hcx = W * 0.72;
      const hcy = H * 0.44;
      const hr  = Math.min(W, H) * 0.13;
      ctx.strokeStyle = D ? "rgba(230,46,4,0.13)" : "rgba(230,46,4,0.09)";
      ctx.lineWidth   = 1;
      ctx.beginPath();
      ctx.arc(hcx, hcy, hr, 0, Math.PI * 2);
      ctx.stroke();
      [0, 60, 120, 180, 240, 300].forEach(deg => {
        const rad = (deg * Math.PI / 180) + hexAngle;
        const dx  = hcx + Math.cos(rad) * hr;
        const dy  = hcy + Math.sin(rad) * hr;
        ctx.shadowColor = "rgba(230,46,4,0.9)";
        ctx.shadowBlur  = 10;
        ctx.fillStyle   = D ? "rgba(230,46,4,0.75)" : "rgba(230,46,4,0.52)";
        ctx.beginPath();
