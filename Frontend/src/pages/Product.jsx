...
import { productsApi, categoriesApi, BASE_URL, getToken } from "../services/api";

// ─── Search tracking helper ───────────────────────────────────────────────────
function logSearch(query, source = "product-page") {
...
// ─── SIDEBAR ──────────────────────────────────────────────────────────────────
function Sidebar({ dark, activeCategory, onSelectCategory, categories }) {
  ...
  return (
    <aside style={{
      ...
      <ul style={{ listStyle: "none", padding: "8px 0" }}>
        {(categories || CATEGORIES_SIDEBAR).map((cat) => (
          <li key={cat.name}>
            <button
...
function ProductsPage() {
  ...
  // Sync activeCategory from URL param — only when no brand/search override is active
  useEffect(() => {
  ...
  // Sync search box value from URL param (e.g. header "See all results" link)
  useEffect(() => {
    setSearchQuery(urlSearch ? decodeURIComponent(urlSearch) : "");
    setCurrentPage(1);
  }, [urlSearch]);

  // When dbCategories loads or activeCategory changes, resolve the categoryId
  useEffect(() => {
    if (activeCategory === "All Products") {
      setActiveCategoryId(null);
    } else {
      const match = dbCategories.find(c => c.name === activeCategory);
      setActiveCategoryId(match ? match.id : null);
    }
  }, [activeCategory, dbCategories]);

  // Main fetch: products from DB
  useEffect(() => {
    setLoading(true);
    ...
    if (searchQuery.trim()) {
      logSearch(searchQuery.trim(), "product-page");
      fetchFn = productsApi.search(searchQuery.trim(), { page, size: ITEMS_PER_PAGE });
    } else if (urlBrand) {
      fetchFn = productsApi.getByBrand(urlBrand, { page, size: ITEMS_PER_PAGE });
    } else if (activeCategoryId) {
      fetchFn = productsApi.getByCategory(activeCategoryId, { page, size: ITEMS_PER_PAGE });
    } else {
      const sortMap = {
        "newest": { sortBy: "createdAt", direction: "desc" },
        "oldest": { sortBy: "createdAt", direction: "asc" },
        "name-asc": { sortBy: "name", direction: "asc" },
        "name-desc": { sortBy: "name", direction: "desc" },
      };
      const s = sortMap[sortBy] || sortMap["newest"];
      fetchFn = productsApi.getAll({ page, size: ITEMS_PER_PAGE, ...s });
    }
    ...
    fetchFn
      .then(data => {
        ...
        setProducts(items);
        setTotalPages(data?.totalPages ?? (items.length < ITEMS_PER_PAGE ? currentPage : currentPage + 1));
        setTotalItems(data?.totalElements ?? items.length);
      })
      .catch(() => { setProducts([]); setTotalPages(1); setTotalItems(0); })
      .finally(() => setLoading(false));
  }, [currentPage, searchQuery, activeCategoryId, sortBy, urlBrand, urlSearch]);

  // Normalize DB product to card format
  const normalizeProduct = (p) => ({
    ...
    img: p.imageUrl
      ...
      : (p.additionalImages?.[0]
          ? (p.additionalImages[0].startsWith("http") ? p.additionalImages[0] : `${BASE_URL}${p.additionalImages[0]}`)
          : ""),
    tag: p.tag || (p.featured ? "Featured" : "ISO Certified"),
    href: `/productDetail?id=${p.id}`,
    price: p.price,
    stock: p.stock,
  });

  // Build sidebar categories from DB (with All Products at top)
  const sidebarCategories = [
    { name: "All Products", count: totalItems },
    ...dbCategories.map(c => ({
      name: c.name,
      count: "", // count per category not easily available without extra calls
      img: c.imageUrl ? (c.imageUrl.startsWith("http") ? c.imageUrl : `${BASE_URL}${c.imageUrl}`) : null,
    }))
  ];
  ...
  const handleCategoryChange = (catName) => {
    ...
    if (catName === "All Products") {
      setActiveCategoryId(null);
      navigate("/products", { replace: true }); // clears any stale ?brand= or ?search=
    } else {
      const match = dbCategories.find(c => c.name === catName);
      setActiveCategoryId(match ? match.id : null);
      navigate(`/products?category=${encodeURIComponent(catName)}`, { replace: true }); // clears stale ?brand=
    }
  };
  ...
  const handleSearch = (q) => {
    setSearchQuery(q);
    setCurrentPage(1);
    // Only touch the URL when a stale brand/category filter needs clearing —
    // avoid pushing history entries on every keystroke.
    if (q.trim() && (urlBrand || urlCategory)) {
      navigate("/products", { replace: true });
    }
  };

  const paginated = products.map(normalizeProduct);
        ...
        <div>
          ...
          {loading && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 24 }}>
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} style={{ borderRadius: 16, overflow: "hidden", background: dark ? "#1a1a1a" : "#fff", border: `1px solid ${dark ? "#2a2a2a" : "#f0f0f0"}` }}>
                  <div style={{ height: 200, background: dark ? "#222" : "#f0f0f0", animation: "shimmer 1.4s infinite" }} />
                  <div style={{ padding: 18 }}>
                    <div style={{ height: 12, borderRadius: 6, background: dark ? "#2a2a2a" : "#eee", marginBottom: 10, width: "40%" }} />
                    <div style={{ height: 16, borderRadius: 6, background: dark ? "#2a2a2a" : "#eee", marginBottom: 6, width: "80%" }} />
                    <div style={{ height: 16, borderRadius: 6, background: dark ? "#222" : "#f5f5f5", width: "60%" }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {!loading && paginated.length === 0 && (
            <div style={{ textAlign: "center", padding: "80px 20px" }}>
              ...
              {urlBrand ? (
                <>
                  <h3 style={{ color: dark ? "#ccc" : "#555", fontSize: 20, fontWeight: 700, marginBottom: 8 }}>
                    No products available for <span style={{ color: "#e62e04" }}>{decodeURIComponent(urlBrand)}</span>
                  </h3>
                  <p style={{ color: dark ? "#666" : "#999", fontSize: 14, marginBottom: 20 }}>
                    We currently don't have any products listed under this brand.
                  </p>
                  <Link to="/products" style={{ display: "inline-block", background: "#e62e04", color: "#fff", padding: "10px 24px", borderRadius: 8, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
                    Browse All Products
                  </Link>
                </>
              ) : (
          ...
          {!loading && totalPages > 1 && (
            <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={p => { setCurrentPage(p); window.scrollTo({ top: 0, behavior: "smooth" }); }} dark={dark} />
          )}
        </div>
      </div>
    </main>
