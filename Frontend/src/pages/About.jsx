  ...
  return (
    <section className="page-hero" style={{ background: "transparent", borderBottom: `1px solid ${dark ? "#222" : "#eee"}`, padding: "40px 0", transition: "background 0.4s" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(16px,4vw,48px)" }}>
        <h1 style={{ fontSize: "clamp(26px,5vw,36px)", fontWeight: 800, color: dark ? "#fff" : "#111", marginBottom: 10, letterSpacing: "-0.5px" }}>About Us</h1>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14, color: dark ? "#888" : "#666" }}>
          ...
          <span>›</span><span style={{ color: dark ? "#ccc" : "#444" }}>About Us</span>
        </div>
      </div>
    </section>
  );
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(16px,4vw,48px)" }}>
        ...
        <div className="about-vision-grid">
          {VISION_MISSION.map(item => (
            <div key={item.label} className="ray-card" style={{ background: visionBg, borderRadius: 16, padding: "clamp(20px,3vw,36px)", borderLeft: "4px solid #e62e04", transition: "background 0.4s" }}>
