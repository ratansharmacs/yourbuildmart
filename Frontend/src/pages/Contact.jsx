...
function PageHero() {
  const { dark } = useTheme();
  return (
    <section style={{ background: "transparent", borderBottom: `1px solid ${dark ? "#1e1e1e" : "#efefef"}`, padding: "clamp(28px,4vw,48px) 0", transition: "background 0.4s" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(16px,4vw,48px)" }}>
        ...
        <h1 style={{ fontSize: "clamp(26px,4vw,38px)", fontWeight: 800, color: dark ? "#f0f0f0" : "#111", letterSpacing: "-0.5px", lineHeight: 1.15 }}>Contact Us</h1>
      </div>
    </section>
  );
}
...
function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await requestsApi.submitContact({
              ...
              <form onSubmit={handleSubmit} noValidate>
                <div className="contact-form-grid" style={{ gap: 16, marginBottom: 0 }}>
                  ...
                  <div style={{ gridColumn: "1 / -1", marginTop: 8 }}>
                    {errors.submit && <p style={{ color: "#e62e04", fontSize: 13, marginBottom: 10 }}>{errors.submit}</p>}
